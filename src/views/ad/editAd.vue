<template>
  <div id="editAd">
    <el-card class="container-wrap">
      <div slot="header">
        <comm-card-header title="广告编辑" iconClass="el-icon-edit-outline"> </comm-card-header>
      </div>
      <div class="container">
        <el-form :model="formData" :rules="rules" ref="form" label-width="120px">
          <el-form-item prop="pic_type" label="投放类型">
            <el-select v-model="formData.pic_type" placeholder="请选择" @change="picTypeChange" :disabled="isLock">
          ...
          <el-form-item prop="tp_id" label="场景分类" v-if="showTpID">
            <el-radio-group v-model="formData.tp_id" @change="tpIDchange" :disabled="isLock">
...
export default {
  data: () => ({
    ...
    rules: {
      pic_type: [{ required: true, message: "请选择投放类型", trigger: "blur" }],
      tp_id: [{ required: true, message: "请选择场景分类", trigger: "blur" }],
      dev_id: [{ required: true, message: "请选择投放场景", trigger: "blur" }],
      file_name: [{ required: true, message: "请上传广告图片", trigger: "blur" }],
      pic_name: [{ required: true, message: "请输入品牌名称", trigger: "blur" }],
      period: [{ required: true, message: "请选择有效期", trigger: "blur" }],
      timeFrame: [{ required: true, message: "请选择投放时段", trigger: "blur" }],
          ...
          this.$confirm("是否提交修改广告?", "提示", {
            ...
            .then(() => {
              const handleParams = {
                ...
                timeFrame: undefined, //清空不需要的参数
                period: undefined
              };
              this.$http.ad
                .editAd(Object.assign(this.formData, handleParams))
                .then(() => {
  ...
  async mounted() {
    const currentEditAd = this.$route.params.editAd;
    if (currentEditAd instanceof Object) {
      await this.getSceneDetail(currentEditAd.tp_id);
      this.formData.pid = currentEditAd.id;
      this.formData.pic_type = currentEditAd.type;
      this.formData.tp_id = currentEditAd.tp_id;
      this.formData.dev_id = currentEditAd.region;
      this.formData.pic_name = currentEditAd.name;
      this.formData.pic_url = currentEditAd.pic_url;
      this.formData.file_name = currentEditAd.pic;
      this.formData.period = [currentEditAd.time3, currentEditAd.time4];
      this.formData.timeFrame = [currentEditAd.time1, currentEditAd.time2];
      this.formData.alone = currentEditAd.alone;
      this.formData.code = currentEditAd.code;

      this.isLock = currentEditAd.is_lock === 1;
      this.selectType = currentEditAd.type;
      this.showFileName = currentEditAd.showPic;
    } else {
      this.$router.push("/ad/listAd");
    }
  },
